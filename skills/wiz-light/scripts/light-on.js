...
/**
 * Wiz-Light: Turn Light ON with Color
 *
 * This script turns on a Wiz light with the specified color.
 * Default color is white.
 *
 * Usage:
 *   node light-on.js [ip] [port] [color] [brightness]
 *
 * Colors:
...
if (!ip || ip === '192.168.1.2') {
  ...
  console.error('Usage: node light-on.js [ip] [port] [color] [brightness]');
  console.error('Example: node light-on.js 192.168.1.2 38899 red 80');
...
async function turnOnLight() {
  try {
    console.log(`\nTurning on light at ${ip} with ${color} color at ${dimming}% brightness...`);

    // Build color props
    const colorProps = { ...lightColor, dimming, state: true };
    ...
    console.error('2. Check if the port is correct:', DEFAULT_PORT);
